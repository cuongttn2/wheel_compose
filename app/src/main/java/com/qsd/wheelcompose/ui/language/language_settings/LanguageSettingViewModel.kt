...
@HiltViewModel
class LanguageSettingViewModel @Inject constructor(@ApplicationContext private val context: Context) :
    ...
    override fun handleIntent(intent: LanguageIntent) {
        ...
        if (intent is LanguageIntent.RefreshLanguage) {
            EventManager.triggerEvent(AppEvent.RefreshAppLanguage)
        }
    }
}